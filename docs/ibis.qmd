...
```{python}
# equivalent to above but longer (not recommended)
...
## ***Order_by*** different variables ascending or descending
...
```{python}
policies.mutate(gender_map = _.gender.case().when('M', 'Male').else_('Female').end())
...
policies.mutate(exposure_group = lambda _: ibis.case() \
    .when(_.exposure > 0.5, 'over') \
    .else_('under') \
    .end())
# note the underscore after else because else is a key work in python
# also don't forget the end function required
```
...
policies2 = policies \
    ...
    .aggregate(exposure = _.exposure.sum(), claim = _.claim.sum()) \
...
- pandas: use for small datasets, or where the ibis API does not allow any complex transformation. 
...
pol.mutate(sex=_.Gender.case() \
    .when('Male', 'M') \
    .when('Female', 'F') \
    .end())
pol.mutate(sex=_.Gender[:1])  # same as above appply str fn
pol.mutate(dead=lambda _: ibis.case() \
    .when(_.Type_of_Event == 'Death', 1) \
    .else_(0) \
    .end())
...
policy_split \
    ...
    .pivot_wider(names_from='Type_of_Event', values_from='Event_Amount_Insurer')
```